...
def hello_http(request):
    ...
    update_stmt = table.update().values(token=random_token, token_exp_time=tkn_exp_time).where(table.c.username == data['user_email'])
...
def usage_demo(email_id,tkn):
    ...
    test_message_text = f"Hello from the Amazon SES mail demo! Click here to register: http://jaswanthmarri.com:8080/v1/user/register?token={tkn}"
    test_message_html = "<p>Hello!</p><p>From the <b>Amazon SES</b> mail demo!</p><p>Click <a href='http://jaswanthmarri.com:8080/v1/user/register?token=" + tkn + "'>here</a> to register.</p>"
